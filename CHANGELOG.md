...
- **macOS release artifacts**: the release workflow now produces a `.app` bundle (with `.icns` icon and `Info.plist`) plus a bare CLI binary for both `x86_64-apple-darwin` and `aarch64-apple-darwin`. Together with the existing Linux and Windows builds, MMCL now ships official artifacts for all three desktop platforms.
- **Windows CLI release artifact**: `mmcl-cli-windows-x86_64.exe` is now uploaded next to `mmcl-windows-x86_64.exe` for users running PowerShell automation, WSL, or scripted workflows.
